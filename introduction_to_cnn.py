...
if __name__ == '__main__':
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.Conv2D(kernel_size=3, filters=64, padding="same", activation=tf.nn.relu,
                               input_shape=(28, 28, 1)),
        tf.keras.layers.MaxPooling2D(pool_size=2),
        tf.keras.layers.Conv2D(kernel_size=5, filters=64, padding="same", activation=tf.nn.relu),
        tf.keras.layers.MaxPooling2D(pool_size=2),
